...
This repository contains versions of the tools I developed for Denial-of-Service mitigation research during [my PhD](https://dspace.mit.edu/handle/1721.1/152862).  It includes scripts for both generating and mitigating volumetric flooding attacks, as well as tools for automating the experimentation process.  Automation tools assume the use of a Merge-based testbed like SPHERE (formerly DeterLab), but files in the `common` directory are general purpose, and should function across most Linux systems.  The next section provides instructions for running experiments in Merge, with documentation for other utilities following.
...
### clear_logs
Deletes and recreates `/tmp/logs`, where devices temporarily store logs during an experiment before they are copied to the XDC.  This is done automatically between each experiment by the `experiment` playbook.
