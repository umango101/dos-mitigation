...
mkdir -p $log_dir
log_file="$log_dir/dns.csv"
source_file="/usr/local/dos-mitigation/common/clients/dns_pow_client.c"
client_bin="/usr/local/dos-mitigation/common/clients/dns_pow_client"

clang -O2 /usr/local/dos-mitigation/common/clients/dns_pow_client.c -o dns_pow_client
...
#    dig @$server_ip $query_domain +norecurse +time=2 +tries=1 > /dev/null
	./dns_pow_client "$server_ip" "$threshold" "$client_ip"
	ok=$?
	end="$(date +%s%N)"
