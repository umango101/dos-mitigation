...
#define RAND_DST_ADDR 1 // Toggle destination address randomization
#define RAND_SRC_PORT 1 // Toggle source port randomization
#define INCREMENT_ID 0 // Toggle incrementing of IP ID field
...
static uint16_t random_port(void) {
	uint16_t port = (uint32_t)(rand()) & 0xff;
	return port;
}

static uint16_t random_id(void) {
	uint16_t id = rand() % (49151 + 1 - 1024) + 1024; // Contrain to registered port range
	return id;
}
...
int main(int argc, char *argv[]) {
  ...
  srand(time(NULL));
	...
	while(1) {
		#if RAND_SRC_ADDR || RAND_DST_ADDR || RAND_SRC_PORT || INCREMENT_ID
			#if RAND_SRC_ADDR
			...
			#if RAND_DST_ADDR
			// Generate a new random destination IP, excluding certain prefixes
			iph->daddr = (__be32)(random_ipv4());
			#endif
