...
const uint32_t QUIC_PACKET_SIZE = 1200;
...
int main(int argc, char *argv[]) {
	...
	iph->version = 4;
	iph->tos = 0;
	iph->tot_len = sizeof (struct iphdr) + sizeof (struct udphdr) + QUIC_PACKET_SIZE;
    ...
    quich->token_len = 0;
    // quich. token = 0; // Token (..),
    quich->len = QUIC_PACKET_SIZE; // Initial packets must be padded to 1200 bytes
    quich->packet_number = 0;

	crypto_frame->frame_type = 6;
	...
	psh.udp_length = htons(sizeof(struct udphdr) + QUIC_PACKET_SIZE);

	int psize = sizeof(struct pseudo_header) + sizeof(struct udphdr) + QUIC_PACKET_SIZE;
