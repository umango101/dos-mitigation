...
for _dev in "${_devs[@]}"; do
  ...
  if [[ $_toggle -eq 1 ]]; then
    ...
    clang -O2 -target bpf -D POW_THRESHOLD=$pow_threshold -c /usr/local/dos_mitigation/common/ebpf/syn_pow.c -o syn_pow\
      -I /usr/include/bpf\
      -I /usr/include/iproute2\
      -I /usr/include/x86_64-linux-gnu\
      -Wno-int-to-void-pointer-cast
      
    /usr/local/dos_mitigation/common/ebpf/bin/tc_load_ingress syn_pow $interface
  fi
done
